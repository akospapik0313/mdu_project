...
def _extract_series(df: pd.DataFrame) -> dict[str, pd.Series | None]:
    ...
    id_price_col = _find_first_existing_column(
        df,
        [
            "id_price_eur_mwh",
            "id_vwap_eur_mwh",
    ...
    return {
        ...
        "id_price": df[id_price_col] if id_price_col else None,
...
def plot_rescheduled_schedule(
    ...
    is_id_plot = (
        "id_rescheduled" in schedule_path.name.lower()
        or "rescheduled/id" in str(schedule_path).replace("\\", "/").lower()
        or s["id_price"] is not None
    )

    stage_label = (
        "Intraday Rescheduled Schedule"
        if is_id_plot
        else "Rescheduled Day-Ahead Schedule"
    )
    ...
    if output_path is None:
        ...
        if is_id_plot:
            output_path = Path(
                f"data/output_data/plots/rescheduled/id/id_rescheduled_plot_{day_str}.png"
            )
        else:
            output_path = Path(
                f"data/output_data/plots/rescheduled/da/da_rescheduled_plot_{day_str}.png"
            )
    ...
    if s["load"] is not None:
        ax.plot(ts, s["load"], linewidth=1.8, label="Load")
        ax.legend(loc="upper right")
    ax.set_ylabel("kW")
    ax.set_title("Expected Load")
    ax.grid(True, alpha=0.3)

    # 2) PV
    ax = axes[1]
    if s["pv"] is not None:
        ax.plot(ts, s["pv"], linewidth=1.8, label="PV generation")
        ax.legend(loc="upper right")
    ax.set_ylabel("kW")
    ax.set_title("Expected PV Generation")
    ax.grid(True, alpha=0.3)

    # 3) Prices
    ax = axes[2]
    plotted_any_price = False
    ...
    if s["dam_price"] is not None:
        ...
        plotted_any_price = True

    if is_id_plot and s["id_price"] is not None:
        ax.plot(
            ts,
            s["id_price"],
            linewidth=1.8,
...
def main() -> None:
    ...
    parser.add_argument(
        "--schedule-path",
        required=True,
        help="Path to the rescheduled schedule file.",
    )
    parser.add_argument(
        "--output-path",
        default=None,
    ...
    saved = plot_rescheduled_schedule(
        schedule_path=args.schedule_path,
        output_path=args.output_path,
    )

    print("\n============================================================")
    print("RESCHEDULED VISUALIZATION")
    print("============================================================")
    print(f"\nPlot saved:\n{saved}")
